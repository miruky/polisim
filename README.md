...
- 「共有リンク」で現在のポリシーとリクエストをURLに畳んで渡せます。「結果をコピー」で判定の要約をテキストで取り出せます
- 配色はOS設定に追従し、ヘッダーのトグルでライト/ダーク/自動を切り替えられます。入力内容はブラウザに保存され、開き直しても続きから扱えます
...
- `src/lib/share.ts` — ポリシーとリクエストのURLフラグメント相互変換
